...
extension TeacherToolX on TeacherTool {
  ...
  /// عنوان زرّ التوليد — وتُذيَّل بـ«✨» كما في التصميم.
  String get generateLabel => switch (this) {
        TeacherTool.lessonPlan => "توليد خطة الدرس",
        TeacherTool.simplify => "تبسيط المفهوم وابتكار تشبيهات",
        TeacherTool.homework => "إنشاء الواجب مع سلّم التصحيح",
        TeacherTool.ask => "",
      };

  /// عنوان بطاقة الإعدادات.
  ///
  /// 🔴 **ولا يُنقل عن المصمّم هنا** (قرار المالك ٢٠٢٦-٠٩-٢١): كتب
  ///    «إعداد خطة التحضير الوزاري» فقال المالك «خطة الدرس خلاص، قل
  ///    إعداد خطة الدرس». والأداةُ تبني خطةً من **درسٍ في الكتاب**، ولا
  ///    علاقة لها بخطة التحضير الوزاريّة الرسمية — فالعنوانُ كان يَعِد
  ///    بما لا يُنجز. وقُصّرت أخواتُه معه.
  String get cardTitle => switch (this) {
        TeacherTool.lessonPlan => "إعداد خطة الدرس",
        TeacherTool.simplify => "تبسيط مفهوم للطلاب",
        TeacherTool.homework => "إعداد واجب واختبار",
...
/// 🏷️ **اسمُ وضعِ المعلّم كما يُقرأ** — لا كما يُخزَّن.
///
/// 🔴 `selectedMode` عند المعلّم مفتاحُ نطاقٍ لا نصُّ شاشة: `"معلم:homework"`.
///    وكان يظهر كما هو تحت عنوان المحادثة في القائمة الجانبية — رأيتُه في
///    المحاكي: «احياء · معلم:homework». مفتاحٌ داخليٌّ أمام معلّم.
///
/// وما ليس مفتاحَ معلّمٍ يعود كما هو (أوضاعُ الطالب مقروءةٌ أصلاً).
String teacherModeLabel(String mode) {
  if (!isTeacherMode(mode)) return mode;
  return TeacherToolX.fromId(mode.substring(kTeacherModePrefix.length))?.label ??
      mode;
}

/// بادئةُ وضعِ المعلّم في مفتاح النطاق — `"معلم:plan"` مثلاً.
const String kTeacherModePrefix = "معلم:";

/// 👨‍🏫 **هل هذا وضعُ معلّمٍ لا وضعُ طالب؟**
///
/// 🔴 **ولمَ يلزم سؤالٌ صريح:** محادثاتُ القسمين تعيش في **صندوقٍ واحد**
///    تحت الحساب نفسِه والصفِّ نفسِه، ويفصلها مفتاحُ النطاق وحده. فأيُّ
///    استعلامٍ يقرأ «كل محادثات هذا الصف» يخلطهما — وقد وقع فعلاً: زرُّ
///    «أكمل من حيث توقفت» في رئيسية **الطالب** التقط محادثةَ «خطة درس»
///    من قسم المعلم (رُصد في المحاكي ٢٠٢٦-٠٩-٢٢)، وبطاقةُ «٥ محادثة»
///    كانت تعدّها معها.
///
/// ⚠️ والدورُ **قابلٌ للتبديل من الإعدادات**، فهذا ليس حالاً نادراً:
///    كلُّ من جرّب القسمين على حسابٍ واحد يحمل النوعين معاً.
bool isTeacherMode(String mode) => mode.startsWith(kTeacherModePrefix);
