...
def _atom(src: str, i: int):
    ...
    while j < len(src) and src[j] not in _BREAK:
        j += 1
    tok = src[i:j]
    # 🔴 **علامةُ نهاية الجملة ليست جزءاً من المقدار**: «ما قيمة جذر ٢٥؟»
    #    كانت تخرج `\sqrt{٢٥؟}` — علامةُ استفهامٍ **داخل الجذر** على شاشة
    #    الطالب. وكشفها اختبارُ «اختبر نفسك» (2026-09-13) لأن السؤال ينتهي
    #    بها دائماً. ونقتطعها من الذيل لا نكسر عندها، كي تبقى النقطةُ
    #    العشرية داخل العدد: «٣.١٤» عددٌ واحد و«جذر ١٦.» ليست كذلك.
    trimmed = tok.rstrip("؟?!.…،؛:\"'»")
    j -= len(tok) - len(trimmed)
    tok = trimmed
    if not tok:
        return None
