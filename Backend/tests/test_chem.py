...
def test_to_ring_keeps_the_book_explanation_after_the_colon():
    """🔒 نحذف **الوصف النثري للشكل** لا **محتوى الكتاب**.

    ⚖️ **وتغيّر موضعُ الترميز في جولة 2026-09-13 عن قصد:** كان يحلّ محلّ
       كلمة «البنزين» (لأنها كانت تُعدّ شكلاً)، فيبقى بعد النقطتين
       «شكل سداسي بداخله دائرة» — وهو **بالضبط** ما يمنع البرومبتُ الموديلَ
       من كتابته. الآن يحلّ محلّ الوصف نفسه، ويبقى اسمُ المركّب عنواناً
       له: «رسمة البنزين : ⬡». فالاسم يُقرأ والرسم يُرى.
    """
    from core.chem import to_ring
    out = to_ring("رسمة البنزين : شكل سداسي بداخله دائرة أو ثلاث روابط ثنائية.")
    assert r"\ring{6|ar}" in out
    assert "شكل سداسي" not in out     # الوصف النثري لم يبقَ منافساً للترميز
    assert "البنزين" in out           # واسمُ المركّب باقٍ كما في الكتاب
